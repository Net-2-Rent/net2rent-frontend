...
export default function LoginForm({ onSubmit, submitError, isSubmitting, onForgot }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form className="login-form" onSubmit={handleSubmit(onSubmit)} noValidate>
      {submitError && (
        <NoticeBanner tone="error">{submitError}</NoticeBanner>
      )}
      ...
      <FormField id="email" label="Correo electrónico" error={errors.email?.message}>
        <TextField
          ...
          {...register('email', {
            ...
            pattern: { value: EMAIL_RE, message: 'El formato del email no es válido' },
          })}
        />
      </FormField>
      ...
      {onForgot && (
        <div className="login-form__forgot-row">
          <button type="button" className="login-form__forgot" onClick={onForgot}>
      ...
      >
        {isSubmitting ? <Spinner size="sm" tone="on-brand" /> : 'Iniciar sesión'}
      </Button>
    </form>
  );
}