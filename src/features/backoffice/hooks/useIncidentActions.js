...
export function useIncidentActions({
  ...
  /**
   * runAction: el patrón try/catch/finally
   *
   * @param {string}   key       identificador de la acción ('reject', 'pause'...)
   * @param {Function} apiCall   función que devuelve la promesa de la API
   * @param {object}   options
   * @param {string}   options.fallback         mensaje si la API no manda uno
   * @param {boolean}  options.minDuration      envuelve con withMinDuration (spinner mínimo)
   * @param {boolean}  options.refreshTimeline  recargar cronología al terminar (por defecto true)
   * @param {boolean}  options.tracksSaved      esta acción maneja el flag "saved" (clasificar/editar)
   * @param {Function} options.onSuccess        efecto extra en éxito (p. ej. cerrar el modal)
   * @param {Function} options.onError          efecto extra en error (p. ej. cerrar modal, reintentar)
   */
