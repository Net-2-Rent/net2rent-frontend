...
export async function addIncidentComment(incidentId, text) {
  const { data } = await httpClient.post(`/incidents/${incidentId}/comments`, {
    text,
  });
  return data;
}
...
export async function pauseIncident(id, reason) {
  const { data } = await httpClient.patch(`/incidents/${id}/pause`, {
    reason: reason?.trim(),
  });
  return data;
}
